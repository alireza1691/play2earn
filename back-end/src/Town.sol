  ...
  /*  ******************************************************************************
    ...
    *******************************************************************************  */
    ...
    function _attack(uint256[] memory warriorsAmounts, uint256 attackerId, uint256 targetId, uint256 wallsLevel) internal  returns(bool, uint256){
        ...
        uint256 attackerHp;
        (,uint256 defenderPower, uint256 defenderHp,) = _getArmyInfo(targetId);
 ...
 contract Town is Barracks{
  ...
  /*  ******************************************************************************
    ...
    *******************************************************************************  */
    ...
    uint256 private constant  WorkerGoldPerMinute = 1 ether ;
    ...
    *******************************************************************************  */
    ...
    function recruit(uint256 landTokenId,uint256[6] memory amounts) external onlyLandOwner(landTokenId){
        ...
        for (uint i = 0; i < amounts.length; i++) {
            ...
            totalFood += BaseWarriorRequiredFood * totalAmount * (i == 5 ? 2 : 1);
            _addWarrior(i, amounts[i], landTokenId); 
        }
        (,,,uint256 currentArmy) = _getArmyInfo(landTokenId);
        if ((landData[landTokenId].trainingCampLevel > 0 ?(landData[landTokenId].trainingCampLevel * BaseArmyCapacity) : 10) < currentArmy + totalAmount) {
            revert MaxCapacity();
        }
    ...
    function dispatchArmy(uint256[] memory warriorAmounts,uint256 from,  uint256 target) external onlyLandOwner(from) {
        ...
        (,,,uint256 totalArmy) = _getArmyInfo(from);
    ...
    function retreat(uint256 landTokenId ,uint256 dispatchedArmyIndex) external onlyLandOwner(landTokenId) {
        ...
        uint256 retreatCost = RetreatCostPerWarrior * dispatchedArmyAmount;
    ...
    function war(uint256 landTokenId ,uint256 dispatchedArmyIndex) external onlyLandOwner(landTokenId) returns(bool, uint256, uint256){
        ...
        (,uint256 defenderPower, uint256 defenderHp,) = _getArmyInfo(dArmy.target);
        ...
        dArmy.remainedTime = block.timestamp + remainingTime;
        if (success) {
            uint256 maxLootCapacity = (dArmy.totalArmyAmount * remainedAttackerArmy/ 100) * BaseWarriorLootCapacity; 
