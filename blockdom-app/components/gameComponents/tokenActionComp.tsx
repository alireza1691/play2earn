...
export default function TokenActionComp() {
  const { tokenCompTab, setTokenCompTab, selectedWindowComponent,setSelectedWindowComponent } = useSelectedWindowContext();
    ...
    return (
        ...
        <div className="inputBg flex flex-row py-1 px-2 gap-2">
          ...
          <input
            ...
            type="number"
            onChange={(event) => setEnteredAmount(Number(event.target.value))}
          ></input>
        </div>
  ...
  const FaucetComp = () => {
    return (
        <div className="flex mt-auto flex-col  px-3">
        <p className="balBg px-3 py-2 blueText text-[14px] !font-normal">
          Claim test BMT and conver them to Gold/Food (Convert them in townhall).
        </p>
      </div>
    )
  }
  ...
  return (
    ...
    { selectedWindowComponent == "tokenActions" && 
       ...
       <div className="h-[2rem] rounded-t-xl px-3 py-1 blueText bg-[#06291D80]/50 flex flex-row items-center justify-between">
         <h3>BMT</h3>
         <button onClick={() => setSelectedWindowComponent(null)} className=" hover:bg-white/10 rounded-lg p-1 transition-all">  <CloseIcon /></button>
       ...
       <div className=" px-4 flex flex-col gap-4 flex-grow">
         {tokenCompTab == "deposit/withdraw" &&     <DepositAndWithdraw/>}
           {tokenCompTab == "faucet" && <FaucetComp/>}
       </div>
       ...
       <div className="mt-auto flex flex-row  px-3 py-3 gap-3 !h-fit">
         ...
         {tokenCompTab == "deposit/withdraw" && isDeposit && (
           <button onClick={() => deposit(approvedAmount)} disabled={approvedAmount <= 0} className="greenButton !w-1/2">
             Deposit
           </button>
         )}
         {tokenCompTab == "deposit/withdraw" && !isDeposit && (
           <button onClick={() => withdraw(approvedAmount)} className="greenButton !w-1/2">Withdraw</button>
         )}
         {tokenCompTab == "faucet" && (
           <button onClick={() => faucet()} className="greenButton !w-1/2">Get BMT</button>
         )}
       </div>
     </section>
    }
