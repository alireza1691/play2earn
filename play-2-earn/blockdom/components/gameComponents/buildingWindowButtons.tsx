...
export default function BuildingWindowButtons() {
  const { selectedItem,  setUpgradeMode, setActiveMode, activeMode, upgradeMode } = useSelectedBuildingContext();
  ...
  async function mintResourceBuilding() {
    try {
      if (signer && inViewLand) {
        ...
        await townSInst(signer).buildResourceBuilding(inViewLand?.tokenId , selectedItem?.name == "Farm" ? 0 : 1)
      }
    } catch (error) {
      console.log(error);
  ...
  return (
    <div className="flex justify-between gap-2 p-2  flex-shrink-0 ">
      {upgradeMode ?(
        <><button onClick={() =>  setUpgradeMode(false) } className="redButton !w-1/2">Back</button>
        <button onClick={() => upgrade()} className="greenButton !w-1/2">Confirm upgrade</button>
        </>
