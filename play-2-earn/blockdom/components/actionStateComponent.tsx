    ...
    const title = () :string => {
        ...
        if (transactionState) {
            ...
            if (     transactionState == "failedConfirmation" ) {
                titleString = "Transaction failed. Please try again or contact support."
            }
  ...
  return (
    ...
    {transactionState && 
      ...
      {transactionState == "txRejected" && (
        <div className='flex flex-grow'>    <div className='flex ml-auto mr-auto w-12 h-auto  mt-auto mb-auto '><RejectIcon/></div></div>

)}
      {transactionState == "failedConfirmation" && (
        <div className='flex flex-grow'>    <div className='flex ml-auto mr-auto w-12 h-auto  mt-auto mb-auto '><FailedIcon/></div></div>

)}
      {transactionState == "confirmed" && (
        <div className='flex flex-grow'>    <div className='flex ml-auto mr-auto w-14 h-auto  mt-auto mb-auto '><DoneIcon/></div></div>
...
{transactionState == "waitingBlockchainConfirmation" && (
    ...
    <BlockdomLogo/>
      ...
      { transactionState == "txRejected" || transactionState == "confirmed" || transactionState == "connectionRejected" || transactionState == "failedConfirmation" ? (
            <div className='mt-auto px-3 py-3 flex flex-shrink'> <button onClick={() => setTransactionState(null) } className=' !py-2 !w-full outlineGreenButton'>Close</button></div>
